...
如果工作区提供 **Extract Data** 操作，它可以让智能体在指定时间范围内处理较大范围的数据来源，并提取结构化信息。操作会按设定的 schema 返回对象列表，供智能体生成答复。上游文档记录的实现最高可处理 500,000 个 token 的来源内容；实际限制可能因工作区和配置而异。
...
2. 选择要处理的数据来源，方式与配置搜索类似。上游指南中的示例包括 Slack 频道、由集成填充的文件夹或客服工单来源。只选择工作区已连接、且智能体使用者有权访问的来源。
