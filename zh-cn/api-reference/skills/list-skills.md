...
列出工作区中的自定义 Skills；默认仅返回启用中的 Skills。
...
| `status` | 查询 | string | 否 | 按状态筛选 Skills；默认值为 active。 可选值：`active`, `archived`, `suggested` |
| `availability` | 查询 | array[string] | 否 | 按可用状态筛选 Skills。可重复传入多个值。只有设置 bypassEditorVisibility 时才会返回未发布的编辑者 Skills。 |
| `bypassEditorVisibility` | 查询 | boolean | 否 | 设为 true 时，同时返回未发布的编辑者 Skills；需要管理员 API key。 |
