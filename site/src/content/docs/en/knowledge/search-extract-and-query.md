---
title: "Choose search, extraction, or table queries"
...
For missing information, check [source selection](/en/knowledge/choose-sources/#choose-knowledge-sources) and [the difference between synchronization and operation tools](/en/integrations/connections-and-tools/#connections-and-tools). For incorrect numbers, check whether text search was used for a quantitative task and whether the table contains duplicates, numbers stored as text, or mixed currencies. Split long periods into explicit batches and reconcile their coverage; do not present a smaller range as a complete result.

[Documentation index](/en/) · [简体中文](/zh-cn/#知识来源与回答核对)
