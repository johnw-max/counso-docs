...
````yaml https://raw.githubusercontent.com/dust-tt/dust/refs/heads/main/front-api/public/swagger.json get /api/w/{wId}/files/{fileId}
openapi: 3.0.0
info:
...
paths:
  /api/w/{wId}/files/{fileId}:
    get:
      ...
      description: >-
        View or download a file. Skill attachments require read access to their
        associated skill. Use query parameters `version` (original, processed,
        public) and `action` (view, download).
      parameters:
        - in: path
          name: wId
          required: true
