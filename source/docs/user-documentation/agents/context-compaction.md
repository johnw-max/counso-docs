...
Use compaction when a conversation has become long or close to its context limit to avoid unexpected interactions pruning. In the composer, the circular context indicator shows the current context usage percentage.
...
2. Click the new context usage indicator in the composer.
3. Click **Compact now**.
