...
原文索引中的 356 个页面和 2 份接口规范均有记录，其中 197 页提供双语正文。数据连接、第三方工具、远程 MCP 接入及通过 OAuth 连接 Counso MCP 的使用说明均在文档范围内。

137 份公开 API 及配套开发资料暂不发布；9 份上游发布历史和已弃用开发框架资料也不作为 Counso 正文发布。另有 15 篇外部插件与自定义导入方案标记为 `hold_integration`：这些主题适用，但原文依赖上游应用、安装包或实验脚本，需要验证 Counso 的对应实现后发布使用教程。它们不属于原生数据连接，也不因公开 API 文档暂缓而被否定。

所有暂未发布的条目均保留原文、原始 URL 和具体原因，不生成占位文章。
