...
| `PUBLICATION-STATUS.md` | 发布建议、需要确认的事项、未确认时可能出现的问题，以及本版不纳入的资料。 |
...
另外 20 篇产品帮助、集成与 CLI 教程已准备中英文稿件，当前 URL 显示“文档更新中”。它们没有被永久排除；发布前需核对内置能力、专用应用、分发包、导入脚本或服务配置与教程一致。当前标记不等于这些功能在 Counso 中均未实现。逐项依据与稿件链接见[文档发布范围与准备稿](PUBLICATION-STATUS.md)。API 文档、JavaScript SDK、客户端 MCP 和 Postman 文件已纳入；9 份原产品更新历史、实验仓库说明和已弃用框架资料不纳入 Counso 文档。
