...
Enumerates rows in table {tId} of data source {dsId} within workspace {wId}.
...
| `limit` | query | integer | No | Limit the number of rows returned |
| `offset` | query | integer | No | Offset the returned rows |
...
| 404 | Table, data source or workspace not found. |
