# List personal consumption analytics facets

Return filter values for the signed-in member’s consumption during the selected period. The server limits this route to that member; user and group dimensions are omitted. Filters on other dimensions determine which values remain available.

```http
POST /api/w/{wId}/me/analytics/consumption/facets
```
