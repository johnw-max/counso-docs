...
Returns the set of agent configurations belonging to workspace {wId}.
...
| `view` | query | string | No | The view to use when retrieving agents:<br>- all: Retrieves all non-private agents (default if not authenticated)<br>- list: Retrieves all active agents accessible to the user (default if authenticated)<br>- published: Retrieves all agents with published scope<br>- global: Retrieves all global agents<br>- favorites: Retrieves all agents marked as favorites by the user (only available to authenticated users)<br>- all_unrestricted: Retrieves every active agent of the workspace, including unpublished agents the caller does not edit and agents requesting spaces the caller cannot access. Requires an admin key.<br> Values: `all`, `all_unrestricted`, `list`, `workspace`, `published`, `global`, `favorites` |
| `withAuthors` | query | string | No | When set to 'true', includes recent authors information for each agent Values: `true`, `false` |

## Responses

| HTTP status | Description |
| --- | --- |
| 200 | Agent configurations for the workspace |
...
| 403 | Forbidden. The all_unrestricted view requires a workspace admin. |
