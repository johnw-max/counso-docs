...
Saving a configuration does not make every external action safe or available. Before sharing an Agent, review its data and access settings and test the expected result with non-sensitive material. For instruction design, read [Write Agent instructions](./write-instructions.md#write-agent-instructions); for access scope, read [Manage Agent data and access](./data-and-access.md#manage-agent-data-and-access).
