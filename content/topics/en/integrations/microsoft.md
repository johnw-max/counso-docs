...
Power BI has its own setup and API permissions. Follow [Power BI](./power-bi.md#power-bi-tool) for Entra app registration, tenant setting, and the MCP connection fields. An Entra app alone is not a working Power BI connection; verify one permitted workspace or model read.
