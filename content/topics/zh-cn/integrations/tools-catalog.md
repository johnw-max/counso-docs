...
使用独立的 [Power BI](./power-bi.md#power-bi-工具) 与 [NetSuite](./netsuite.md#netsuite-工具) 页面。Power BI 需要 Entra 应用、委派权限、租户 MCP 设置和受限模型读取。NetSuite 需要 SuiteCloud 功能、MCP SuiteApp、专用角色和 OAuth 集成记录；不要使用 Administrator 角色。
...
Agent Memory、委派和定时提示词的配置，请参阅[高级工作方式](../agents/advanced-work-modes.md#选择高级助手工作方式)。
...
参阅[连接与工具](./connections-and-tools.md#连接与工具)、[个人与共享访问](./personal-and-shared.md#个人与共享授权)和[远程 MCP](./remote-mcp.md#添加远程-mcp-服务器)。
